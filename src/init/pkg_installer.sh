...
tar czf ${WAZUH_HOME}/backup/backup_${VERSION}_[${BDATE}].tar.gz -C ${WAZUH_HOME}/tmp_bkp . >> ${WAZUH_HOME}/logs/upgrade.log 2>&1
...
if [ "$status" = "connected" -a $RESULT -eq 0  ]; then
    ...
    tar xzf ${WAZUH_HOME}/backup/backup_${VERSION}_[${BDATE}].tar.gz -C / >> ${WAZUH_HOME}/logs/upgrade.log 2>&1
